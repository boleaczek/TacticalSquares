...
pub mod calculations {
    ...
    pub fn get_point_of_intersection(line_a: LineEquation, line_b: LineEquation) -> Option<Coordinates> {
        match line_a {
            LineEquation::Vertical(x) => return calculate_intersection_for_vertical(x, &line_b),
            LineEquation::Horizontal(y) => return calculate_intersection_for_horizontal(y, &line_b),
            LineEquation::Curve{slope, y_intercept} => return calculate_intersection_for_curve(slope, y_intercept, &line_b)
        }
    }
    ...
    fn calculate_intersection_for_vertical(x: f64, line_b: &LineEquation) -> Option<Coordinates> {
        match line_b {
            ...
            LineEquation::Curve{slope, y_intercept} => Some(curve_x_intersection(*slope, *y_intercept, x))
        }
    }
    ...
    fn calculate_intersection_for_horizontal(y: f64, line_b: &LineEquation) -> Option<Coordinates> {
        match line_b {
            ...
            LineEquation::Curve{slope, y_intercept} => return Some(curve_y_intersection(*slope, *y_intercept, y))
        }
    }

    fn calculate_intersection_for_curve(slope_a: f64, y_intercept_a: f64, line_b: &LineEquation) -> Option<Coordinates> {
        match line_b {
            LineEquation::Vertical(x) => return Some(curve_x_intersection(slope_a, y_intercept_a, *x)),
            LineEquation::Horizontal(y) => return Some(curve_y_intersection(slope_a, y_intercept_a, *y)),
            LineEquation::Curve{slope, y_intercept} => return Some(curve_curve_intersection(slope_a, y_intercept_a, *slope, *y_intercept))
        }
    }
    ...
    mod tests {
        ...
        #[test]
        fn get_line_equation_coordinates_for_vertical_vertical_returned() {
            let a = Coordinates::new(0.0, 1.0);
            let b = Coordinates::new(0.0, 2.0);

            let equation = get_line_equation(&a, &b);
        ...
        #[test]
        fn get_line_equation_coordinates_for_horizontal_horizontal_returned() {
        ...
        #[test]
        fn get_line_equation_coordinates_for_curve_provided_curve_returned() {
            let a = Coordinates::new(0.0, 1.0);
        ...
        #[test]
        fn get_point_of_intersection_curves_provided_returns_point_of_intersection() {
            let curve_a = LineEquation::Curve{slope: 2.0, y_intercept: 0.0};
            let curve_b = LineEquation::Curve{slope: -1.0, y_intercept: 6.0};

            let result = get_point_of_intersection(curve_a, curve_b);
        ...
        #[test]
        fn get_point_of_intersection_non_corssing_vertical_lines_provided_returns_none() {
            let line_a = LineEquation::Vertical(0.0);
            let line_b = LineEquation::Vertical(1.0);

            let result = get_point_of_intersection(line_a, line_b);

            assert_eq!(result.is_none(), true);
        }

        #[test]
        fn get_point_of_intersection_non_corssing_horizontal_lines_provided_returns_none() {
        ...
        #[test]
        fn get_point_of_intersection_vertical_and_horizontal_lines_provided_returns_point_of_intersection() {
        ...
        #[test]
        fn get_point_of_intersection_curve_and_vertical_line_provided_returns_point_of_intersection() {
            let curve = LineEquation::Curve{slope: 2.0, y_intercept: 1.0};
            let line = LineEquation::Vertical(2.0);

            let result = get_point_of_intersection(curve, line);
        ...
        #[test]
        fn get_point_of_intersection_curve_and_horizontal_line_provided_returns_point_of_intersection() {
            let curve = LineEquation::Curve{slope: 2.0, y_intercept: 1.0};
